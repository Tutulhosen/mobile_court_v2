...
class DashboardController2 extends Controller
{
    ...
    public function monthlyReport(Request $request)
    {
        ...
        // Add conditions based on user profile
        if ($roleID == 37 || $roleID == 38) {
            $condition .= "  AND l.Level2IDOld', '=', $zillaId";
        }
        //   dd($condition);
        // elseif ($profile == 'Divisional Commissioner') {
        //     $condition .= " AND l.Level1IDOld = $divId";
        // }

        // // Fetch the data dynamically using DB query
        // $resultset = DB::table('monthly_report as l')
        //     ->select('*')  // You can specify the necessary columns
        //     ->whereYear('l.date', $year)
        //     ->whereRaw('1=1',$condition)  // Dynamic conditions
        //     ->whereMonth('l.date', $currentMonth)
        //     ->orWhereMonth('l.date', $previousMonth)
        //     ->toSql();
        ...
        $resultset =  $this->fetchDataTableBySql_new($condition, $year, $currentMonth, $previousMonth ,$zillaId, $roleID);
        // dd( $resultset);
        return response()->json($resultset);
    }

    public function fetchDataTableBySql_new($condition, $year, $current_month, $previous_month, $zillaId, $roleID)
    {
        $previousYear = '';
        $currentYear = '';
        $previousYearMonth = '';
        ...
        $result="SELECT 
            ...
            if ($roleID == 37 || $roleID == 38) {
                ...
                $result .= " WHERE l.Level2IDOld = $zillaId";
            }
            ...
            return [
                ...
                '9' => $item->PCaseCount,
                '10' => $item->PCourtCount,
                '11' => $item->PCriminalCount,
                '12' => $item->PFineCount,
                '13' => $item->PJailCount,
                '14' => $item->CCaseCount,
                '15' => $item->CCourtCount,
                '16' => $item->CCriminalCount,
                '17' => $item->CFineCount,
                '18' => $item->CJailCount,
                'CCaseCount' => $item->CCaseCount,
                'CCourtCount' => $item->CCourtCount,
                'CCriminalCount' => $item->CCriminalCount,
                'CFineCount' => $item->CFineCount,
                'CJailCount' => $item->CJailCount,
                ...
                'LocationType' => 'UPAZILLA',
                'PCaseCount' => $item->PCaseCount,
                'PCourtCount' => $item->PCourtCount,
                'PCriminalCount' => $item->PCriminalCount,
                'PFineCount' => $item->PFineCount,
                'PJailCount' => $item->PJailCount,
    ...
    public function fetchDataTableBySql($condition, $year, $current_month, $previous_month, $zillaId)
    {

        
        $current_month = $current_month; // Example current month
        $year = $year; // Example current year

        // Determine the previous and current year and month
        if ($current_month == 1) {
            $previousYear = $year - 1;
            $currentYear = $year;
            $previousYearMonth = 12;
            ...
            $previousYear = $year;
            $currentYear = $year;
            $previousYearMonth = $current_month - 1;
            $currentYearMonth = $current_month;
        }
        ...
        // Main query
        $result = DB::table('upazila as u')
            ->select(
                'u.district_id as ZillaID',
                'u.upazila_name_bn as ZillaName',
           
                DB::raw('IFNULL(j1.PCourtCount, 0) AS PCourtCount'),
                DB::raw('IFNULL(j1.CCourtCount, 0) AS CCourtCount'),
                DB::raw('IFNULL(j2.PCaseCount, 0) AS PCaseCount'),
                DB::raw('IFNULL(j2.CCaseCount, 0) AS CCaseCount'),
                DB::raw('IFNULL(j3.PCriminalCount, 0) AS PCriminalCount'),
                DB::raw('IFNULL(j3.CCriminalCount, 0) AS CCriminalCount'),
                DB::raw('IFNULL(j3.PFineCount, 0) AS PFineCount'),
                DB::raw('IFNULL(j3.CFineCount, 0) AS CFineCount'),
                DB::raw('IFNULL(j3.PJailCount, 0) AS PJailCount'),
                DB::raw('IFNULL(j3.CJailCount, 0) AS CJailCount')
            )
            ->leftJoin(
                DB::raw("(SELECT 
       
                p.zillaId, 
                COUNT(CASE WHEN (YEAR(p.date) = $previousYear AND MONTH(p.date) = $previousYearMonth) THEN 1 END) AS PCourtCount, 
                COUNT(CASE WHEN (YEAR(p.date) = $currentYear AND MONTH(p.date) = $currentYearMonth) THEN 1 END) AS CCourtCount 
            FROM prosecutions AS p 
            JOIN courts AS c ON c.id = p.court_id 
            WHERE p.delete_status = 1 AND c.status = 2 AND c.delete_status = 1 
            AND (YEAR(c.date) = $previousYear OR YEAR(c.date) = $currentYear) 
            AND (MONTH(c.date) = $previousYearMonth OR MONTH(c.date) = $currentYearMonth) 
            GROUP BY  p.zillaId) j1"),
                'j1.zillaId',
                '=',
                'u.district_id'
            )
            ->leftJoin(
                ...
                p.zillaId, 
                COUNT(CASE WHEN (YEAR(p.date) = $previousYear AND MONTH(p.date) = $previousYearMonth) THEN 1 END) AS PCaseCount, 
                COUNT(CASE WHEN (YEAR(p.date) = $currentYear AND MONTH(p.date) = $currentYearMonth) THEN 1 END) AS CCaseCount 
            FROM prosecutions AS p 
            WHERE p.delete_status = 1 AND p.is_approved = 1 AND p.orderSheet_id IS NOT NULL 
            AND (YEAR(p.date) = $previousYear OR YEAR(p.date) = $currentYear) 
            AND (MONTH(p.date) = $previousYearMonth OR MONTH(p.date) = $currentYearMonth) 
            GROUP BY  p.zillaId) j2"),
                'j2.zillaId',
                '=',
                'u.district_id'
            )
            ->leftJoin(
                ...
                p.zillaId, 
                COUNT(CASE WHEN (YEAR(P.date) = $previousYear AND MONTH(P.date) = $previousYearMonth) THEN 1 END) AS PCriminalCount, 
                COUNT(CASE WHEN (YEAR(P.date) = $currentYear AND MONTH(P.date) = $currentYearMonth) THEN 1 END) AS CCriminalCount, 
                SUM(CASE WHEN (YEAR(P.date) = $previousYear AND MONTH(P.date) = $previousYearMonth) THEN IFNULL(pn.fine, 0) ELSE 0 END) AS PFineCount, 
                SUM(CASE WHEN (YEAR(P.date) = $currentYear AND MONTH(P.date) = $currentYearMonth) THEN IFNULL(pn.fine, 0) ELSE 0 END) AS CFineCount, 
                COUNT(CASE WHEN (YEAR(P.date) = $previousYear AND MONTH(P.date) = $previousYearMonth) THEN IFNULL(pn.punishmentJailID, 0) ELSE 0 END) AS PJailCount, 
                COUNT(CASE WHEN (YEAR(P.date) = $currentYear AND MONTH(P.date) = $currentYearMonth) THEN IFNULL(pn.punishmentJailID, 0) ELSE 0 END) AS CJailCount 
            FROM punishments AS pn 
            JOIN prosecutions AS P ON P.id = pn.prosecution_id 
            WHERE P.delete_status = 1 AND P.is_approved = 1 AND P.orderSheet_id IS NOT NULL 
            AND pn.criminal_id IS NOT NULL 
            AND (YEAR(P.date) = $previousYear OR YEAR(P.date) = $currentYear) 
            AND (MONTH(P.date) = $previousYearMonth OR MONTH(P.date) = $currentYearMonth) 
            GROUP BY  P.zillaId) j3"),
                'j3.zillaId',
                '=',
                'u.district_id'
            )
            ->where('u.district_id', $zillaId)
            ->orderBy('u.district_id')
            ->get();
        ...
        $formattedResult = $result->map(function ($item) {
            return [
                ...
                5 => $item->PCaseCount,
                6 => $item->CCaseCount,
                7 => $item->PCriminalCount,
                8 => $item->CCriminalCount,
                9 => $item->PFineCount,
                10 => $item->CFineCount,
                11 => $item->PJailCount,
                12 => $item->CJailCount,
    ...
    public function ajaxDashboardCriminalInformation(Request $request)
    {
        ...
        if ($roleID == 37 or $roleID == 38) {
            ...
            list($zillaid, $block_label) = $this->setBlockLabelForADM($upozilaid, $GeoCityCorporations, $GeoMetropolitan, $GeoThanas);

            $childs = DashboardCitizenRepository::getCitizenComplainStatistics('', $zillaid, $upozilaid, $GeoCityCorporations, $GeoMetropolitan, $GeoThanas, $end_date, $start_date);

            // } elseif ($profile == 'Divisional Commissioner') {

            //     list($divid, $block_label) = $this->setBlockLabelForDivisionalCommissioner($session_arr,$zillaid, $upozilaid, $GeoCityCorporations, $GeoMetropolitan, $GeoThanas);
            //     $childs = $this->dashboardcitizen->getCitizenComplainStatistics($divid, $zillaid, $upozilaid, $GeoCityCorporations, $GeoMetropolitan, $GeoThanas, $end_date, $start_date);
    ...
    public function ajaxdashboardCaseStatistics(Request $request)
    {
        ...
        if ($roleID == '37' || $roleID == '38') {
            ...
            list($result1, $result2, $result3) = $this->getingQueryResults('', $zillaid, $upozilaid, $GeoCityCorporations, $GeoMetropolitan, $GeoThanas, $start_date, $end_date);

            $result4 = DashboardCitizenRepository::getCountOfProsecutor('', $zillaid, $upozilaid, $GeoCityCorporations, $GeoMetropolitan, $GeoThanas, $start_date, $end_date);

            $childs = $this->setDataToChildArray($result1, $childs, $result2, $result3);
            $childs['prosecutor_no'] =  BanglaDate::bangla_number($result4);
        }
    ...
    public function ajaxCitizen(Request $request)
    {
        ...
        if ($roleID == 37 || $roleID == 38) {
            ...
            list($zillaid, $block_label) = $this->setBlockLabelForADM($upozilaid, $GeoCityCorporations, $GeoMetropolitan, $GeoThanas);
            $childs = DashboardCitizenRepository::getDataCitizenComplainForZilla($zillaid, $upozilaid, $GeoCityCorporations, $GeoMetropolitan, $GeoThanas, $start_date, $end_date);
            //    dd($childs );
            // } elseif ($profile == "Divisional Commissioner") {

            //     list($divid, $block_label) = $this->setBlockLabelForDivisionalCommissioner($session_arr,$zillaid, $upozilaid, $GeoCityCorporations, $GeoMetropolitan, $GeoThanas);
            //     $childs = $this->dashboardcitizen->getDataCitizenComplainForDiv($divid,$zillaid, $upozilaid, $GeoCityCorporations, $GeoMetropolitan, $GeoThanas,$start_date,$end_date );

            // } elseif ($profile == 'JS') {

            //     $block_label = $this->setBlockLabelForJS($divid,$zillaid, $upozilaid, $GeoCityCorporations, $GeoMetropolitan, $GeoThanas);
            //     $childs = $this->dashboardcitizen->getDataCitizenComplainForCountry($divid,$zillaid, $upozilaid, $GeoCityCorporations, $GeoMetropolitan, $GeoThanas,$start_date,$end_date );
    ...
    public function ajaxDataFineVSCase(Request $request)
    {
        ...
        if ($roleID == 37 || $roleID == 38) {
            ...
            list($zillaid, $block_label) = $this->setBlockLabelForADM($upozilaid, $GeoCityCorporations, $GeoMetropolitan, $GeoThanas);
            $childs = DashboardFine::getDataFineVSCaseZilla($zillaid, $upozilaid, $GeoCityCorporations, $GeoMetropolitan, $GeoThanas, $start_date, $end_date);
            //   return $childs;
            // } elseif ($profile == "Divisional Commissioner") {

            //     list($divid, $block_label) = $this->setBlockLabelForDivisionalCommissioner($session_arr,$zillaid, $upozilaid, $GeoCityCorporations, $GeoMetropolitan, $GeoThanas);
            //     $childs = $this->dashboardFine->getDataFineVSCaseDiv($divid, $zillaid, $upozilaid, $GeoCityCorporations, $GeoMetropolitan, $GeoThanas, $start_date, $end_date);

            // } elseif ($profile == 'JS') {

            //     $block_label = $this->setBlockLabelForJS($divid,$zillaid, $upozilaid, $GeoCityCorporations, $GeoMetropolitan, $GeoThanas);
            //     $childs = $this->dashboardFine->getDataFineVSCaseCountry($divid, $zillaid, $upozilaid, $GeoCityCorporations, $GeoMetropolitan, $GeoThanas, $start_date, $end_date);
    ...
    public function ajaxDataLocationVSCase(Request $request)
    {
        ...
        if ($roleID == 37 || $roleID == 38) {
            ...
            list($zillaid, $block_label) = $this->setBlockLabelForADM($upozilaid, $GeoCityCorporations, $GeoMetropolitan, $GeoThanas);
            $childs = DashboardLocationRepository::getDataLocationVSCaseForZilla($zillaid, $upozilaid, $GeoCityCorporations, $GeoMetropolitan, $GeoThanas, $start_date, $end_date);
            // dd($childs);
            // } elseif ($profile == "Divisional Commissioner") {

            //     list($divid, $block_label) = $this->setBlockLabelForDivisionalCommissioner($session_arr,$zillaid, $upozilaid, $GeoCityCorporations, $GeoMetropolitan, $GeoThanas);
            //     $childs = $this->dashboardlocation->getDataLocationVSCaseForDiv($divid, $zillaid, $upozilaid, $GeoCityCorporations, $GeoMetropolitan, $GeoThanas,$start_date,$end_date );

            // } elseif ($profile == 'JS') {

            //     $block_label = $this->setBlockLabelForJS($divid,$zillaid, $upozilaid, $GeoCityCorporations, $GeoMetropolitan, $GeoThanas);
            //     $childs = $this->dashboardlocation->getDataLocationVSCaseForCountry($divid, $zillaid, $upozilaid, $GeoCityCorporations, $GeoMetropolitan, $GeoThanas,$start_date,$end_date );
    ...
    public function ajaxDataLawVSCase(Request $request)
    {
        ...
        if ($roleID == 37 || $roleID == 38) {
            ...
            list($zillaid, $block_label) = $this->setBlockLabelForADM($upozilaid, $GeoCityCorporations, $GeoMetropolitan, $GeoThanas);
            $childs = Dashboardlaw::getDataLawVSCaseZilla($zillaid, $upozilaid, $GeoCityCorporations, $GeoMetropolitan, $GeoThanas, $start_date, $end_date);

            // dd($childs);
            // } elseif ($profile == "Divisional Commissioner") {

            //     list($divid, $block_label) = $this->setBlockLabelForDivisionalCommissioner($session_arr,$zillaid, $upozilaid, $GeoCityCorporations, $GeoMetropolitan, $GeoThanas);
            //     $childs = $this->dashboardlaw->getDataLawVSCaseDiv($divid, $zillaid, $upozilaid, $GeoCityCorporations, $GeoMetropolitan, $GeoThanas, $start_date, $end_date);

            // } elseif ($profile == 'JS') {

            //     $block_label = $this->setBlockLabelForJS($divid,$zillaid, $upozilaid, $GeoCityCorporations, $GeoMetropolitan, $GeoThanas);
            //     $childs = $this->dashboardlaw->getDataLawVSCaseForCountry($divid, $zillaid, $upozilaid, $GeoCityCorporations, $GeoMetropolitan, $GeoThanas, $start_date, $end_date);
    ...
    public function getAjaxUrlParameters(Request $request)
    {
