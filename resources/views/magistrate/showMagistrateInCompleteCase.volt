...
{% for it in page.items %}
    {% if loop.first %}
        ...
        <caption class="caption-custom">অনিষ্পন মামলার   তালিকা</caption>
        <thead>
        <tr>
            {#<th>Id</th>#}
            <th>নম্বর </th>
    ...
    <tr>
        ...
        {#<td width="12%">{{ link_to("prosecution/newComplain/" ~ it.id,  'অভিযোগ গঠন ', "class": "btn") }}</td>#}
        {#<td width="12%">{{ link_to("prosecution/delete/" ~ it.id, 'জব্দতালিকা', "class": "btn") }}</td>#}
    </tr>
    </tbody>
    {% if loop.last %}
        <tbody>
        <tr>
            <td colspan="12" align="center">
                <div class="btn-group">
                    {{ link_to("magistrate/showMagistrateInCompleteCase", '<i class="icon-fast-backward"></i> প্রথম', "class": "btn") }}
                    {{ link_to("magistrate/showMagistrateInCompleteCase?page=" ~ page.before, '<i class="icon-step-backward"></i> আগে', "class": "btn ") }}
                    {{ link_to("magistrate/showMagistrateInCompleteCase?page=" ~ page.next, '<i class="icon-step-forward"></i> পরে', "class": "btn") }}
                    {{ link_to("magistrate/showMagistrateInCompleteCase?page=" ~ page.last, '<i class="icon-fast-forward"></i> শেষ', "class": "btn") }}
