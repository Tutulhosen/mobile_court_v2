...
<div id="printcourtvisitreportWithComment" class="content_form_potrait">
    {#{{ stylesheet_link('css/protibedon.css') }}#}
    <div class="form_top_title">
        <table style="width: 100%">
            <tr>
                <td colspan="3" class="centertext"><span
                            id="cmt_courtvisit_allmonthreg_name">allmonthreg_name</span></td>
            </tr>
            <tr>
                {#<td> <h3><?php echo $this->bangladate->get_bangla_monthbymumber(3)?> </h3></td>#}
                <td colspan="3" class="centertext"><span id="cmt_courtvisit_month_year">month_year</span></td>
            </tr>
        </table>
    </div>


    <table class="newfont"   id='cmt_courtvisit_monthlyregister_table' border="1" style="border-collapse:collapse;" cellpadding="2px"
...
<script>
    function pcv_setParamsWithComment(data) {

        document.getElementById("cmt_courtvisit_allmonthreg_name").innerHTML = data.name;
        document.getElementById("cmt_courtvisit_month_year").innerHTML = data.month_year;

        var myTable = document.getElementById("cmt_courtvisit_monthlyregister_table");
        ...
        $(data.result).each(function (index, element) {
            rowcount++;
            var count = element['visit_count'] - 1 ;
            if (count >= 1) {
                comment1 = 'হয়েছে';
            } else {
                comment1 = 'হয়নি';
            ...
            if(rowcount == 38 )
            {
                $('#cmt_courtvisit_monthlyregister_table').append(
                        '<tr>' +
                                '<td colspan="6"> <div style="page-break-after:always;height: 10px"></div></td>' +
                                '</tr>' +
                ...
                if (zillaname == element['zillaname']) {
                    ...
                    $('#cmt_courtvisit_monthlyregister_table').append(
                            '<tr>' +
                                    ...
                                    '<td > ' + totaltext + '</td>' +
                                    '<td class="centertext"> ' + element['visit_promap'] + '</td>' +
                                    '<td class="centertext"> ' +  element['visit_count'] + '</td>' +
                                    '<td class="centertext"> ' + "-" + '</td>' +
                                    '<td class="centertext"> ' + "-" + '</td>' +
                                    '</tr>');
                } else {
