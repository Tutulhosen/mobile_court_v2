...
<div id="printcourtvisitreport" class="content_form_potrait">
    {#{{ stylesheet_link('css/protibedon.css') }}#}
    <div class="form_top_title">
        <table style="width: 100%">
            <tr>
                <td colspan="3" class="centertext"><span
                            id="courtvisit_allmonthreg_name">allmonthreg_name</span></td>
            </tr>
            <tr>
                {#<td> <h3><?php echo $this->bangladate->get_bangla_monthbymumber(3)?> </h3></td>#}
                <td colspan="3" class="centertext"><span id="courtvisit_month_year">month_year</span></td>
            </tr>
        </table>
    </div>


    <table class="newfont"   id='courtvisit_monthlyregister_table' border="1" style="border-collapse:collapse;" cellpadding="2px"
...
<script>
    function pcv_setParams(data) {

        document.getElementById("courtvisit_allmonthreg_name").innerHTML = data.name;
        document.getElementById("courtvisit_month_year").innerHTML = data.month_year;

        var myTable = document.getElementById("courtvisit_monthlyregister_table");
        ...
        $(data.result).each(function (index, element) {
            ...
            if(rowcount == 38 )
            {
                $('#courtvisit_monthlyregister_table').append(
                        '<tr>' +
                ...
                if (zillaname == element['zillaname']) {
                    $('#courtvisit_monthlyregister_table').append(
                            '<tr>' +
                                    ...
                                    '<td class="centertext"> ' + total_visit + '</td>' +
                                    '<td class="centertext"> ' + "-" + '</td>' +
                                    '<td class="centertext"> ' + "-" + '</td>' +
                                    '</tr>');
                    total_visit = 0;
