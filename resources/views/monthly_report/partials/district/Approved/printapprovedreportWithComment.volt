<div id="printapprovedreportWithComment" class="content_form_potrait">
    {{ stylesheet_link('css/protibedon.css') }}
    <div class="form_top_title">
        <table style="width: 100%">
            <tr>
                <td colspan="3" class="centertext"><span
                            id="cmt_approve_allmonthreg_name">allmonthreg_name</span></td>
            </tr>
            <tr>
                {#<td> <h3><?php echo $this->bangladate->get_bangla_monthbymumber(3)?> </h3></td>#}
                <td colspan="3" class="centertext"><span id="cmt_approve_month_year">month_year</span></td>
            </tr>
        </table>
    </div>


    <table id='cmt_approvedreport_table' border="1" style="border-collapse:collapse;" cellpadding="2px"
...
<script>
    function p_approvedrep_setParamsWithComment(data) {

        document.getElementById("cmt_approve_allmonthreg_name").innerHTML = data.name;
        document.getElementById("cmt_approve_month_year").innerHTML = data.month_year;


        var myTable = document.getElementById("cmt_approvedreport_table");
...
//            alert(JSON.stringify(element));
        ...
        if(element['re1'] == 1){
            ...
            $('#cmt_approvedreport_table').append(
                    '<tr>' +
